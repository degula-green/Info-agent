...
> 本文是字段级评审稿，不是最终建表 SQL。相较初版由 42 张表压缩为 26 张表。
...
| `knowledge` | 连接器 / 采集 / 知识管理服务 | 个人连接器、已接入会话的外部身份、采集、消息、附件、知识条目、共享请求、隐私状态和可靠事件 |
| `rag` | 信息处理 / 检索 / 问答服务 | 解析与索引任务、ES 索引映射、搜索历史、问答会话、消息和可靠事件 |
...
9. SHA-256 字段统一保存 64 位小写十六进制字符串，不带 `sha256:` 前缀。

### 1.3 一期核心约束

- 一期一个用户最多存在一个有效组织成员关系；结构保留未来多组织能力。
- 一个组织可以有多个 Owner；同一成员可以同时拥有多个固定角色；Owner 可以转让。
- 一期角色代码固定为 `owner`、`information_admin`、`membership_approver`；有效成员隐式拥有 `member` 基础权限，不建立显式 `member` 角色关系。
...
消息批次共享使用 `private.conversation.share.requested` 事件；`request_id` 是整个 API 请求的幂等键，`share_batch_id` 由服务端首次处理时生成，`source_private_item_id` 指向每个被共享的私人消息知识条目。服务先持久化 `request_id` 与规范化请求指纹，再在独立事务中校验全部 `message_ids` 并创建组织逻辑引用；任意一条失败则整批共享数据回滚，但请求记录保留为 `failed`。相同 `request_id` 携带不同消息集合时返回 `IDEMPOTENCY_CONFLICT`。事件完成后，组织逻辑副本的 `source_type` 为 `shared_private_item`，其 `knowledge_scope='organization'`、`access_scope='organization_members'`。共享不创建持续绑定，不影响私聊后续采集，也不会自动共享后续消息。
...
用途：保存系统预置的组织管理角色定义，不支持组织自定义角色。

| 字段名 | 字段类型 | 主键 | 可空 | 默认值 | 索引 / 约束 | 注释 |
|---|---|---:|---:|---|---|---|
| `id` | UUID | 是 | 否 | `gen_random_uuid()` | 主键 | 角色 ID |
| `code` | VARCHAR(64) | 否 | 否 | 无 | 唯一；CHECK | `owner`、`information_admin`、`membership_approver` |
| `name` | VARCHAR(128) | 否 | 否 | 无 | 无 | 角色名称 |
| `description` | TEXT | 否 | 是 | `NULL` | 无 | 角色说明 |
| `created_at` | TIMESTAMPTZ | 否 | 否 | `CURRENT_TIMESTAMP` | 无 | 创建时间 |

### 2.4 `iam.permissions`（权限定义表）

用途：保存按业务动作划分的系统权限定义。

| 字段名 | 字段类型 | 主键 | 可空 | 默认值 | 索引 / 约束 | 注释 |
|---|---|---:|---:|---|---|---|
| `id` | UUID | 是 | 否 | `gen_random_uuid()` | 主键 | 权限 ID |
| `code` | VARCHAR(128) | 否 | 否 | 无 | 唯一 | 权限代码 |
| `name` | VARCHAR(128) | 否 | 否 | 无 | 无 | 权限名称 |
| `description` | TEXT | 否 | 是 | `NULL` | 无 | 权限说明 |
| `created_at` | TIMESTAMPTZ | 否 | 否 | `CURRENT_TIMESTAMP` | 无 | 创建时间 |

### 2.5 `iam.role_permissions`（角色权限关系表）

用途：保存系统角色到业务权限的固定映射。

| 字段名 | 字段类型 | 主键 | 可空 | 默认值 | 索引 / 约束 | 注释 |
|---|---|---:|---:|---|---|---|
| `role_id` | UUID | 联合主键 | 否 | 无 | 外键 → `iam.roles.id` | 角色 |
| `permission_id` | UUID | 联合主键 | 否 | 无 | 外键 → `iam.permissions.id` | 权限 |
| `created_at` | TIMESTAMPTZ | 否 | 否 | `CURRENT_TIMESTAMP` | 无 | 创建时间 |

### 2.6 `iam.organizations`（组织表）
...
### 2.7 `iam.organization_memberships`（组织成员关系表）
...
### 2.8 `iam.membership_roles`（成员角色表）
...
| `role_id` | UUID | 否 | 是 | `NULL` | 外键 → `iam.roles.id` | 角色定义；历史兼容期间可为空 |
...
关键约束：`UNIQUE (membership_id, role_code) WHERE revoked_at IS NULL`。有效 membership 隐式拥有 `member`，不在本表保存；组织至少有一个有效 Owner 由 IAM 服务在加锁事务中校验。

### 2.9 `iam.organization_invitations`（组织邀请表）
...
### 2.10 `iam.access_requests`（受保护资源访问申请与授权表）
...
| `content_hash` | CHAR(64) | 否 | 否 | 无 | 索引；CHECK | 统一消息内容 SHA-256，64 位小写十六进制 |
...
| `external_attachment_id` | VARCHAR(255) | 否 | 是 | `NULL` | `(message_id, external_attachment_id)` 部分唯一索引 | 平台消息中的外部附件 ID；本地上传为空 |
| `object_ref` | VARCHAR(512) | 否 | 否 | 无 | 普通索引 | 原附件对象引用；逻辑附件可复用同一物理对象 |
| `source_private_attachment_id` | UUID | 否 | 是 | `NULL` | 索引 | 共享私人文件时指向私人附件；普通附件为空 |
| `content_hash` | CHAR(64) | 否 | 否 | 无 | 索引；CHECK | 文件 SHA-256，64 位小写十六进制 |
| `content_version` | INTEGER | 否 | 否 | `1` | CHECK `>= 1` | 一期固定 1 |
...
| `content_hash` | CHAR(64) | 否 | 否 | 无 | 索引；CHECK | 当前业务内容哈希，64 位小写十六进制 |
...
共享幂等约束：`UNIQUE (share_request_id, source_private_item_id)`，允许同一请求共享多条不同私人内容，但同一私人内容不会重复创建组织副本。`knowledge.private_share_requests` 以 `(requester_user_id, request_id)` 保存请求指纹和服务端生成的 `share_batch_id`；相同请求 ID 携带不同消息集合时返回 `IDEMPOTENCY_CONFLICT`。
...
### 3.12 `knowledge.private_share_requests`（私聊共享请求幂等表）

用途：记录一次私聊批量共享请求的指纹、批次状态和结果，保证重试及并发请求不会重复创建组织侧逻辑引用。

| 字段名 | 字段类型 | 主键 | 可空 | 默认值 | 索引 / 约束 | 注释 |
|---|---|---:|---:|---|---|---|
| `id` | UUID | 是 | 否 | `gen_random_uuid()` | 主键 | 记录 ID |
| `requester_user_id` | UUID | 否 | 否 | 无 | 联合唯一 | 请求用户；由认证上下文取得 |
| `request_id` | VARCHAR(100) | 否 | 否 | 无 | 联合唯一 | API 请求幂等键 |
| `request_fingerprint` | CHAR(64) | 否 | 否 | 无 | CHECK | 私聊 ID 与规范化消息 ID 集合的 SHA-256 |
| `private_conversation_id` | UUID | 否 | 否 | 无 | 外键；索引 | 私聊会话；服务层必须校验确属私人会话 |
| `organization_id` | UUID | 否 | 否 | 无 | 无 | 首次处理时确定的目标组织逻辑 ID |
| `share_batch_id` | UUID | 否 | 否 | 无 | 唯一索引 | 第一次处理时由服务端生成 |
| `status` | VARCHAR(16) | 否 | 否 | `'processing'` | 索引；CHECK | `processing`、`completed`、`failed` |
| `shared_message_count` | INTEGER | 否 | 否 | `0` | CHECK `>= 0` | 已共享消息数量 |
| `shared_attachment_count` | INTEGER | 否 | 否 | `0` | CHECK `>= 0` | 已共享附件数量 |
| `last_error` | TEXT | 否 | 是 | `NULL` | 无 | 最近失败摘要 |
| `created_at` | TIMESTAMPTZ | 否 | 否 | `CURRENT_TIMESTAMP` | 无 | 创建时间 |
| `updated_at` | TIMESTAMPTZ | 否 | 否 | `CURRENT_TIMESTAMP` | 状态索引 | 更新时间 |
| `completed_at` | TIMESTAMPTZ | 否 | 是 | `NULL` | 无 | 完成时间 |

共享请求登记与批量 KnowledgeItem 写入不应放在同一个可整体回滚的事务中。先提交请求指纹，再在同一请求锁下执行批量事务；成功更新为 `completed`，失败更新为 `failed`。重试时指纹相同则按状态返回，指纹不同返回 `IDEMPOTENCY_CONFLICT`。
...
| `parsed_content_hash` | CHAR(64) | 否 | 是 | `NULL` | CHECK | 解析产物哈希，64 位小写十六进制 |
...
| `content_variant` | VARCHAR(16) | 否 | 否 | `'display'` | CHECK | `display` 或 `protected`；分别对应展示和受保护 ES 索引 |
...
关键约束：`UNIQUE (knowledge_item_id, content_version, content_variant)`；展示和受保护内容分别记录对应版本，受保护记录只能对应受保护 ES 索引。
...
### 4.6 `rag.outbox_events`（RAG 事务事件表）

用途：由 RAG 服务可靠发布解析和处理结果事件。RAG 只能写本表，不得写 `knowledge.outbox_events`。
...
| `aggregate_type` | VARCHAR(64) | 否 | 否 | 无 | 联合索引 | 聚合类型 |
| `aggregate_id` | UUID | 否 | 否 | 无 | 联合索引 | 聚合 ID |
| `event_type` | VARCHAR(100) | 否 | 否 | 无 | 索引；CHECK | `document.extracted`、`processing.completed`、`processing.failed` |
| `event_version` | BIGINT | 否 | 否 | `1` | 联合唯一；CHECK | 聚合事件版本 |
| `schema_version` | INTEGER | 否 | 否 | `1` | CHECK `>= 1` | 事件载荷版本 |
| `organization_id` | UUID | 否 | 是 | `NULL` | 索引 | 组织上下文；私人任务为空 |
| `trace_id` | VARCHAR(100) | 否 | 否 | 无 | 索引 | 链路 ID |
| `payload` | JSONB | 否 | 否 | `'{}'::jsonb` | 无 | 稳定事件载荷，不放二进制 |
| `status` | VARCHAR(32) | 否 | 否 | `'pending'` | 组合索引；CHECK | `pending`、`publishing`、`published`、`failed` |
| `retry_count` | INTEGER | 否 | 否 | `0` | CHECK `>= 0` | 重试次数 |
| `available_at` | TIMESTAMPTZ | 否 | 否 | `CURRENT_TIMESTAMP` | 组合索引 | 下次可发布时间 |
| `published_at` | TIMESTAMPTZ | 否 | 是 | `NULL` | 无 | 发布时间 |
| `last_error` | TEXT | 否 | 是 | `NULL` | 无 | 最近错误 |
| `created_at` | TIMESTAMPTZ | 否 | 否 | `CURRENT_TIMESTAMP` | 索引 | 创建时间 |

关键约束：`UNIQUE (aggregate_type, aggregate_id, event_type, event_version)`；事件类型限制为上述三个 RAG 处理结果事件。
...
生产使用独立读写别名：
`knowledge_display_chunks_read/write` 和
`knowledge_protected_chunks_read/write`。展示索引保存普通/脱敏内容及附件元数据；受保护索引保存受 OpenFGA `view` 授权约束的原始消息和附件解析内容。禁止先对受保护索引全量召回再过滤权限，查询必须先按授权对象集合过滤。不要按组织创建物理索引，组织和个人隔离通过字段过滤及最终 IAM Check 完成。Mapping、分词器或向量维度发生不兼容变化时，按索引分别创建新版本并原子切换对应别名。完整字段与 Mapping 以 `docs/es-index-design.md` 为准。
...
| `knowledge_item_id`、`content_version`、`auth_acl_version` | 回源、版本一致性和权限复核 |
...
| `title`、`content` | BM25 全文检索；展示索引存普通/脱敏文本，受保护索引存获授权后可检索的原始文本 |
| `embedding` | 向量召回 |
| `content_hash`、`content_visibility` | 一致性和内容可见性校验 |
...
5. 私人消息批次共享：校验所有者、组织成员资格以及全部 `message_ids` 均属于该私聊，以 `(requester_user_id, request_id)` 和规范化请求指纹保证幂等，并按 `share_request_id + source_private_item_id` 创建组织逻辑引用；一次请求可混合选择文字、文件和图片消息，未选中的消息不得创建组织引用。请求登记先提交，批量写入失败时保留 `failed` 指纹记录。
...
|---|---|
| `iam.roles` | 系统预置角色定义 |
| `iam.permissions` | 按业务动作定义的系统权限 |
| `iam.role_permissions` | 角色到权限的固定映射 |
...
| 3 | `iam.roles` | 系统预置角色定义 |
| 4 | `iam.permissions` | 系统权限定义 |
| 5 | `iam.role_permissions` | 角色权限映射 |
| 6 | `iam.organizations` | 组织主体 |
| 7 | `iam.organization_memberships` | 组织成员及退出审批生命周期 |
| 8 | `iam.membership_roles` | 成员固定多角色关系 |
| 9 | `iam.organization_invitations` | 一次性组织邀请 |
| 10 | `iam.access_requests` | 敏感原文 / 受保护附件申请、审批和授权 |
| 11 | `iam.audit_logs` | IAM 和权限审计 |
...
| 12 | `knowledge.private_share_requests` | 私聊共享请求指纹、批次和状态 |
...
| 6 | `rag.outbox_events` | RAG 解析与处理结果事务事件 |
...
| `sharing.share_batch_id` | `knowledge_items.share_batch_id` 与 `private_share_requests.share_batch_id`，表示本次通过 `message_ids` 选择的一组文字、文件或图片消息；第一次处理时由服务端生成 |
