...
class Module2KnowledgeClient(KnowledgeSource):
    ...
    def _get(self, path: str, *, content_version: int | None = None) -> dict[str, Any]:
        if not self.base_url:
            raise KnowledgeSourceUnavailable("RAG_KNOWLEDGE_BASE_URL is not configured")
        try:
            value = self.http.request(
                "GET",
                with_query(join_url(self.base_url, path), content_version=content_version),
                token=self.token,
    ...
    def get_knowledge(self, knowledge_item_id: str, *, content_version: int | None = None) -> dict[str, Any]:
        return self._get(f"/internal/knowledge/{knowledge_item_id}", content_version=content_version)

    def get_content(self, knowledge_item_id: str, *, content_version: int | None = None) -> dict[str, Any]:
        return self._get(f"/internal/knowledge/{knowledge_item_id}/content", content_version=content_version)

    def get_attachment(self, attachment_id: str, *, content_version: int | None = None) -> dict[str, Any]:
        return self._get(f"/internal/attachments/{attachment_id}", content_version=content_version)
