...
class HttpClient:
    """Small stdlib HTTP adapter shared by all internal/cloud clients."""
    ...
    def request(
        ...
        *,
        ...
        try:
            ...
            # Do not include remote response bodies: they may contain secrets or
            # the document text that this service must keep out of logs/errors.
            raise IntegrationError(f"remote HTTP request failed ({exc.code})", status=exc.code, retryable=retryable) from exc
        except (urllib.error.URLError, TimeoutError, OSError) as exc:
            raise IntegrationError("remote HTTP request failed", retryable=True) from exc
        if result.status < 200 or result.status >= 300:
            raise IntegrationError(f"remote HTTP request failed ({result.status})", status=result.status, retryable=result.status >= 500)
        return result
