...
func TestIngestFiltersBeforeDedupeAndDedupeSkipsRepositoryNormalization(t *testing.T) {
	...
	if _, err := f.service.IngestMessage(context.Background(), changed); apperror.From(err).Code != "external_id_conflict" || counting.calls.Load() != 1 {
		t.Fatalf("Redis dedupe did not reject conflicting payload: calls=%d err=%v", counting.calls.Load(), err)
	}
}

func TestUselessPrivateMessagesAreFilteredBeforeRedis(t *testing.T) {
	f := newPipelineFixture(t, domain.PlatformWechat)
	counting := &ingestCountingRepository{Repository: f.repo}
	probe := &dedupeProbeKV{Store: f.store}
	f.service.Repo = counting
	f.service.KV = probe
	...
	for _, test := range []struct {
		...
		{id: "emoji-only", content: "👋🏻 ❤️ 1️⃣"},
		{id: "robot-heartbeat", content: "<heartbeat interval=\"30\">"},
	} {
		result, err := f.service.IngestMessage(context.Background(), pipelineInput(f, test.id, "text", test.content))
		if err != nil || !result.Discarded {
			t.Fatalf("%s was not discarded: result=%+v err=%v", test.id, result, err)
		}
	}
	if counting.calls.Load() != 0 || probe.gets.Load() != 0 || probe.acquires.Load() != 0 {
		t.Fatalf("filtered messages reached dedupe or repository: repo=%d get=%d acquire=%d", counting.calls.Load(), probe.gets.Load(), probe.acquires.Load())
	}
}

func TestUselessBodyWithAttachmentKeepsAttachmentOnly(t *testing.T) {
	f := newPipelineFixture(t, domain.PlatformWechat)
	for index, content := range []string{"👋", "robot heartbeat"} {
		id := fmt.Sprintf("attachment-filter-%d", index)
		result, err := f.service.IngestMessage(context.Background(), pipelineInput(f, id, "image", content, repository.AttachmentInput{
			ExternalAttachmentID: id + "-image", FileName: "image.jpg", MIMEType: "image/jpeg",
		}))
		if err != nil || result.Discarded || len(result.Attachments) != 1 {
			t.Fatalf("attachment was discarded with useless body: result=%+v err=%v", result, err)
		}
	}
	messages, err := f.repo.ListMessages(context.Background(), f.conversation.ID, 10, "")
	if err != nil || len(messages) != 2 {
		t.Fatalf("attachment containers were not saved: messages=%+v err=%v", messages, err)
	}
	for _, message := range messages {
		if message.Content != "" {
			t.Fatalf("useless attachment body was persisted: %+v", message)
		}
	}
}

func TestConcurrentMessageReplayUsesSingleRepositoryWrite(t *testing.T) {
	f := newPipelineFixture(t, domain.PlatformWechat)
	blocking := &blockingIngestRepository{Repository: f.repo, entered: make(chan struct{}), release: make(chan struct{})}
	f.service.Repo = blocking
	input := pipelineInput(f, "concurrent-dedupe", "text", "same content")
	...
	case <-time.After(time.Second):
		t.Fatal("first ingest did not reach repository")
	}
	go func() {
		result, err := f.service.IngestMessage(context.Background(), input)
		outcomes <- outcome{result: result, err: err}
	...
	for range 2 {
		...
		case got := <-outcomes:
			if got.err != nil || got.result == nil {
				t.Fatalf("concurrent ingest failed: result=%+v err=%v", got.result, got.err)
			}
			if got.result.Duplicate {
				duplicates++
			}
		case <-time.After(time.Second):
			t.Fatal("concurrent ingest timed out")
		}
	}
	if blocking.calls.Load() != 1 || duplicates != 1 {
		t.Fatalf("SETNX did not serialize replay: repository calls=%d duplicates=%d", blocking.calls.Load(), duplicates)
	}
}

func TestRedisDedupeFailureFallsBackToDatabaseIdempotency(t *testing.T) {
	f := newPipelineFixture(t, domain.PlatformWechat)
	counting := &ingestCountingRepository{Repository: f.repo}
	f.service.Repo = counting
	f.service.KV = &failingDedupeKV{Store: f.store, err: errors.New("redis unavailable")}
	input := pipelineInput(f, "redis-down", "text", "persist me")

	first, err := f.service.IngestMessage(context.Background(), input)
	if err != nil || first.Duplicate {
		t.Fatalf("first ingest was lost during Redis outage: result=%+v err=%v", first, err)
	}
	second, err := f.service.IngestMessage(context.Background(), input)
	if err != nil || !second.Duplicate || counting.calls.Load() != 2 {
		t.Fatalf("database did not provide final idempotency: result=%+v calls=%d err=%v", second, counting.calls.Load(), err)
	}
	messages, err := f.repo.ListMessages(context.Background(), f.conversation.ID, 10, "")
	if err != nil || len(messages) != 1 {
		t.Fatalf("Redis outage created duplicate rows: messages=%+v err=%v", messages, err)
	}
}
