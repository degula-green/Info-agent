<template>
  <section class="attachment-preview" aria-label="附件预览">
    <div v-if="loading" class="attachment-preview__state"><t-icon name="loading" />正在加载预览…</div>
...
async function load() { release(); if (!props.active) return; loading.value = true; error.value = ''; const ext = extension(); isImage.value = ['png', 'jpg', 'jpeg', 'gif', 'webp', 'svg'].includes(ext); isPdf.value = ext === 'pdf'; try { const blob = await getKnowledgeAttachmentContent(props.file.id); if (isImage.value || isPdf.value) blobUrl.value = URL.createObjectURL(blob); else textContent.value = await blob.text() } catch { error.value = '附件预览暂不可用' } finally { loading.value = false } }
watch(() => [props.active, props.file.id, props.file.type], load, { immediate: true })
onUnmounted(release)
</script>
