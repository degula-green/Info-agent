<template>
  <section class="search-page">
    <div class="search-page__header"><div><h1>知识库搜索</h1><p>跨知识库检索消息与附件，保留来源上下文。</p></div><span class="search-page__hint"><kbd>Ctrl</kbd><kbd>K</kbd> 随时打开搜索</span></div>
    <div class="search-box"><t-icon name="search" /><input ref="inputRef" v-model="query" placeholder="输入关键词，搜索可见知识库中的消息和附件。" @keydown.enter="runSearch" /><t-button theme="primary" :loading="loading" @click="runSearch">搜索</t-button></div>
...
const error = ref('')
const emptyHint = ref('请尝试更短的关键词，或更换平台筛选。')
const total = ref(0)
const page = ref(1)
const pageSize = 20
...
function filteredAll() {
  if (platform.value === 'all') return allResults.value
  return allResults.value.filter((item) => item.platform === platform.value)
}
...
async function runSearch(nextPage: number | Event = 1) {
  ...
  // Re-filter locally when only the page changed and we already have a full hit set.
  if (targetPage !== 1 && allResults.value.length && page.value !== targetPage) {
  ...
  try {
    ...
    allResults.value = mapRagSearchItems(response.items)
    emptyHint.value = allResults.value.length ? '请尝试更短的关键词，或更换平台筛选。' : searchEmptyHint(response.diagnostics)
    applyPlatformFilter()
    store.addRecentSearch(normalized)
  } catch (err: any) {
    if (isAbortError(err)) return
    allResults.value = []
    results.value = []
    total.value = 0
    emptyHint.value = '请尝试更短的关键词，或更换平台筛选。'
    error.value = err?.message || '搜索服务暂不可用'
  } finally {
