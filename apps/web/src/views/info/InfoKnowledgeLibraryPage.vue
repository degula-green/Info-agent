<template>
  <section class="library-page">
    ...
    <div class="library-toolbar">
      <t-input v-model="query" clearable placeholder="搜索名称、消息或文件" @enter="loadItems">
        <template #prefix-icon><t-icon name="search" /></template>
      </t-input>
      <t-select v-model="platform" clearable placeholder="全部平台" class="platform-filter" @change="loadItems">
        <t-option value="feishu" label="飞书" />
        <t-option value="wechat" label="个人微信" />
        <t-option value="wecom" label="企业微信" />
      </t-select>
      <span class="library-toolbar__count">{{ filteredItems.length }} 项</span>
    </div>
    ...
    <div v-else class="library-empty">
      ...
      <h2>{{ query ? '没有匹配内容' : emptyTitle }}</h2>
      <p>{{ query ? '尝试更换关键词或清除筛选条件。' : emptyDescription }}</p>
...
const filteredItems = computed(() => {
  ...
  return items.value.filter((item) => `${item.title} ${item.excerpt || ''} ${item.conversation_name || ''} ${item.file_name || ''}`.toLowerCase().includes(needle))
})
...
async function loadItems() {
  ...
  try { items.value = await getKnowledgeLibraryItems(library.value!.id, { kind: isConversationLibrary.value ? 'conversations' : 'files', platform: platform.value, query: query.value, limit: 200 }) } catch (e: any) { error.value = e?.message || '目录加载失败' } finally { loading.value = false }
}
...
async function attach(session: AvailableConversationDTO) {
  ...
  try {
    const chat = await store.accessTypedSession(discovery.value.platform, isPrivateLibrary.value ? 'private' : 'group', session, discovery.value.discovery_id)
...
async function handleUpload(event: Event) {
  ...
  try {
    ...
    const task = await createLocalUploadTask({ requestID, uploadDestination: props.libraryKind === 'organization_files' ? 'organization_file_library' : 'private_local_library', fileName: file.name, mimeType: file.type || 'application/octet-stream', sizeBytes: file.size, contentHash: hash })
...
@media (max-width: 700px) { .library-page { padding: 24px 16px 45px; }.library-page__header { align-items: stretch; flex-direction: column; }.library-page__actions { justify-content: flex-start; }.library-toolbar { align-items: stretch; flex-wrap: wrap; }.library-toolbar :deep(.t-input) { max-width: none; flex-basis: 100%; }.platform-filter { width: calc(100% - 64px); }.library-toolbar__count { align-self: center; margin-left: auto; }.library-row { align-items: flex-start; }.library-row__meta { display: none; }.library-row__arrow { margin-top: 9px; } }
</style>
